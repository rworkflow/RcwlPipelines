...
```{r}
bwa <- cwlLoad(mcols(tl)$rpath)
bwa
```

Or we can install the tools by its `title` directly.
```{r}
bwa <- cwlLoad(title(tl), bfc = tools)
```
...
```{r}
invisible(sapply(title(tls), cwlInstall, bfc = tools))
```
...
# Pipelines examples
There are over 20 pipelines collected in this package. Here is a
brief introduction to three of them. More pipelines and tools are
...
```{r}
bwaMRecal <- cwlLoad("pl_bwaMRecal", bfc = tools)
inputs(bwaMRecal)
```
...
1. ``r names(runs(bwaMRecal))[[1]]``: bwa alignment by read groups.
```{r}
...
2. ``r names(runs(bwaMRecal))[[2]]``: Markduplicates by `picard`.
```{r}
runs(bwaMRecal)[[2]]
```

3. ``r names(runs(bwaMRecal))[[3]]``: Alignment recalibration by `GATK` toolkit.
```{r}
...
The final processed bam files, bam index, duplicates matrix, and flag
statistics summary will be in the output folder.
...
<https://hubentu.github.io/Rcwl/application.html#dnaseq-alignment-pipeline>
...
```{r}
rnaseq_Sf <- cwlLoad(rname = "pl_rnaseq_Sf", bfc = tools)
inputs(rnaseq_Sf)
```
...
<https://hubentu.github.io/Rcwl/application.html#rnaseq-pipeline>
